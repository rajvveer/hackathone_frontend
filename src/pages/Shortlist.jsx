    ...
    return (
        <div className="space-y-6 pb-20 md:pb-6">
            ...
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                <div>
                    <h1 className="text-2xl md:text-3xl font-bold font-display">Your Shortlist</h1>
                ...
                <div className="card bg-gradient-to-r from-emerald-500/10 to-teal-500/10 border-emerald-500/30">
                    <div className="flex items-center justify-between flex-wrap gap-4">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <p className="text-sm text-emerald-400 font-medium">Locked University</p>
                                <h3 className="text-xl font-semibold">{lockedUni.uni_name}</h3>
                                <p className="text-gray-400">{lockedUni.country}</p>
                            </div>
                        </div>
                        <div className="flex gap-2">
                            ...
                            <button onClick={handleUnlock} className="btn btn-ghost text-amber-400">
                                🔓 Unlock
                            </button>
                        </div>
                    </div>
            ...
            {universities.length === 0 && (
                <div className="card text-center py-12">
                    ...
                    <h3 className="text-lg font-semibold mb-2">No Universities Shortlisted</h3>
            ...
            {['Dream', 'Target', 'Safe'].map(category => {
                ...
                const { icon, color } = styles[category];

                return (
                    <div key={category}>
                        <div className="flex items-center gap-2 mb-4">
                            <span className="text-xl">{icon}</span>
                            <h2 className="text-lg font-semibold">{category} Universities</h2>
                            <span className="badge bg-white/10 text-gray-400">{unis.length}</span>
                        </div>
                ...
                <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4 z-50 animate-fade-in">
                    <div className="card max-w-md w-full animate-slide-up">
                        <div className="text-center mb-6">
                            <div className="text-5xl mb-4">🔒</div>
                            <h3 className="text-xl font-semibold mb-2">Lock {showLockModal.uni_name}?</h3>
...
const ShortlistCard = ({ university, onRemove, onLock, isLocked, hasLockedUni, color, compareMode, isSelectedForCompare, onToggleCompare }) => {
    const chanceColors = {
        Low: 'text-red-400',
    ...
    return (
        ...
        >
            ...
            <div className="flex items-start justify-between gap-2 mb-3">
                <div>
                    <h3 className="font-semibold">{university.uni_name}</h3>
                    <p className="text-sm text-gray-400">{university.country}</p>
                </div>
                {isLocked && <span className="badge badge-success">🔒 Locked</span>}
            </div>

            {university.why_fits && (
                <p className="text-sm text-gray-400 mb-3 line-clamp-2">{university.why_fits}</p>
            )}
                ...
                <div className="flex gap-2">
                    {!isLocked && (
                        <>
                            <button
                                onClick={onRemove}
                                className="btn btn-ghost text-red-400 text-sm flex-1"
                            >
                                Remove
                            </button>
                            {!hasLockedUni && (
...
const CompareModal = ({ universities, onClose, onLock, hasLockedUni }) => {
    ...
    const compareItems = [
        ...
        { label: 'Acceptance Chance', key: 'acceptance_chance' },
        { label: 'Why It Fits', key: 'why_fits' },
    ];

    const getChanceColor = (chance) => {
        const colors = { Low: 'text-red-400', Medium: 'text-amber-400', High: 'text-emerald-400' };
        return colors[chance] || 'text-gray-400';
    };
    ...
    return (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4 z-50 animate-fade-in overflow-y-auto">
            <div className="card max-w-4xl w-full animate-slide-up my-8">
                <div className="flex items-center justify-between mb-6">
                    <h3 className="text-xl font-semibold">⚖️ Compare Universities</h3>
                    <button onClick={onClose} className="btn btn-ghost">✕</button>
                </div>

                {/* Header Row */}
                <div className="grid grid-cols-3 gap-4 mb-4">
                    <div className="font-medium text-gray-400">Criteria</div>
                    <div className="text-center">
                        <h4 className="font-semibold text-lg">{uni1.uni_name}</h4>
                        <span className={`badge ${uni1.category === 'Dream' ? 'badge-dream' : uni1.category === 'Target' ? 'badge-target' : 'badge-safe'}`}>
                            {uni1.category}
                        </span>
                    </div>
                    <div className="text-center">
                        <h4 className="font-semibold text-lg">{uni2.uni_name}</h4>
                ...
                <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-3 gap-4 py-3 border-t border-white/10">
                        ...
                        <div className="text-center">{uni1.country}</div>
                        <div className="text-center">{uni2.country}</div>
                    </div>

                    {/* Acceptance Chance */}
                    <div className="grid grid-cols-3 gap-4 py-3 border-t border-white/10">
                        <div className="text-gray-400">Acceptance Chance</div>
                    ...
                    <div className="grid grid-cols-3 gap-4 py-3 border-t border-white/10">
                        ...
                        <div className="text-center font-semibold">{uni1.data?.fit_score || 'N/A'}/10</div>
                        <div className="text-center font-semibold">{uni2.data?.fit_score || 'N/A'}/10</div>
                    </div>

                    {/* Why It Fits */}
                    <div className="grid grid-cols-3 gap-4 py-3 border-t border-white/10">
                        <div className="text-gray-400">Why It Fits</div>
                        <div className="text-sm text-gray-300">{uni1.why_fits || 'N/A'}</div>
                        <div className="text-sm text-gray-300">{uni2.why_fits || 'N/A'}</div>
                    </div>

                    {/* Key Risks */}
                    <div className="grid grid-cols-3 gap-4 py-3 border-t border-white/10">
                        <div className="text-gray-400">Key Risks</div>
                        <div className="text-sm text-amber-400/80">{uni1.key_risks || 'None noted'}</div>
                        <div className="text-sm text-amber-400/80">{uni2.key_risks || 'None noted'}</div>
                    </div>
                </div>
                ...
                {!hasLockedUni && (
                    <div className="mt-6 pt-4 border-t border-white/10">
                        <p className="text-sm text-gray-400 mb-4 text-center">Ready to commit? Lock one of these universities:</p>
